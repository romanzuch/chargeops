...
describe("signAccessToken", () => {
  it("returns a compact JWS (three dot-separated parts)", async () => {
    const token = await signAccessToken({ userId: "u1", tenantId: "t1" }, SECRET, TTL);
  ...
  it("respects the provided ttlSeconds", async () => {
    const token = await signAccessToken({ userId: "u", tenantId: "t" }, SECRET, 300);
    const payload = await verifyAccessToken(token, SECRET);
...
describe("verifyAccessToken", () => {
  ...
  it("throws UnauthorizedError for an expired token", async () => {
    vi.useFakeTimers();
    const token = await signAccessToken({ userId: "u", tenantId: "t" }, SECRET, 60);
  ...
  it("throws UnauthorizedError for a tampered signature", async () => {
    const token = await signAccessToken({ userId: "u", tenantId: "t" }, SECRET, TTL);
