 ...
 * `tid` is a custom claim carrying the tenant identifier.
 */
export interface AccessTokenPayload {
  /** Subject — the authenticated user's ID. */
  sub: string;
  /** Tenant ID — the tenant scope for this token. */
  tid: string;
 ...
 *
 * @param input     userId + tenantId to embed as claims
...
export async function signAccessToken(
  ...
  return new SignJWT({ tid: input.tenantId })
...
export async function verifyAccessToken(
  ...
  if (typeof tid !== "string" || tid === "") {
    throw new UnauthorizedError("Invalid token: missing tid");
  }
